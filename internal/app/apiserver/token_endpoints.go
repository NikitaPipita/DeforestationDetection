...
func (s *server) updateIotToken() http.HandlerFunc {
	type request struct {
		Token string `json:"token"`
	}
	...
	return func(w http.ResponseWriter, r *http.Request) {
		...
		if ok && token.Valid {
			...
			iotId, err := strconv.ParseUint(fmt.Sprintf("%.f", claims["iot_id"]), 10, 64)
			if err != nil {
				s.error(w, r, http.StatusUnprocessableEntity, err)
				return
			}
			...
			ts, createErr := CreateIotToken(iotId)
			if createErr != nil {
				s.error(w, r, http.StatusForbidden, createErr)
				return
			}

			saveErr := CreateAuth(iotId, ts)
			if saveErr != nil {
				s.error(w, r, http.StatusForbidden, saveErr)
				return
			}
